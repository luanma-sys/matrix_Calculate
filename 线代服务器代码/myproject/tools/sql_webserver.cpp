...
/*
struct st_clientifno
{
  int  sockid;  // 客户端的socket。
  char ip[16];  // 客户端的ip地址。
};
*/
...
int main(int argc, char* argv[])
{
    ...
    for (int ii = 0; ii < 10; ii++)
    {
        ...
        stpthinfo.atime = time(0);        // 把结程的活动时间置为当前时间。
        vthid.push_back(stpthinfo);     // 把线程结构体保存到vthid容器中。
    }
...
void* thmain(void* arg)
{
    ...
    while (true)
    {
        ...
        if (ReadT(connfd, strrecvbuf, sizeof(strrecvbuf), 30000) <= 0) { close(connfd); continue; }
...
void _help(char* argv[])
{
    ...
    printf("Sample:/project/tools1/bin/procctl 10 /project/tools1/bin/webserver /log/idc/sql_webserver.log \"<connstr>127.0.0.1,root,ucp4WaC0tE_s,mat-storage,3306</connstr><charset>utf8</charset><port>8081</port>\"\n\n");
...
bool ExecSQL(connection* conn, const char* buffer, const int sockfd)
{
    //判断是否有Referer字段，如果有则说明是重复链接,返回true是因为处理的不是想要的链接
    if (strstr(buffer, "Referer") != NULL) return true;
