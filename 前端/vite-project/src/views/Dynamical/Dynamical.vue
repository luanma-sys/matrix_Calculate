<template>
  <div style="height:100vh">
    <el-tabs v-model="activeName" class="demo-tabs">
      <el-tab-pane label="离散动力系统" name="first"></el-tab-pane>
    </el-tabs>
    ...
    <div>
      ...
      <el-button @click="mat_Save">保存结果</el-button>
    </div>
    <el-dialog v-model="dialogVisible" title="Shipping address">
      <el-form :model="tableDate">
        <el-form-item label="Promotion name">
          <el-input v-model="tableDate.name" autocomplete="off" />
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="dialogVisible = false">Cancel</el-button>
          <el-button type="primary" @click="mat_Save">
            Confirm
          </el-button>
        </span>
      </template>
...
if (JSON.stringify(route.query) !== "{}") {
  tableDate.matrix[0] = stringToMat(route.query.matmajor)
  tableDate.basic_matrix[0] = stringToMat(route.query.matminor)
  tableDate.name = route.query.matname
}

const mat = ref(null)
onMounted(() => {
  document.getElementsByClassName('dynasize')[0].addEventListener('mousedown', function (e) { global.bindMouseDown(e, mat.value) })
})

function stringToMat(table) {
  var result = {
    id: 1,
    row: 3,
    col: 3,
  ...
  result.col = array[1]
  var mat = []
  for (var row = 0; row < result.row; row++) {
    var temp = []
    for (var col = 0; col < result.col; col++) {
      temp.push(array[2 + row * result.col + col])
    }
    mat.push(temp)
  }
  result.matrix = mat
...
function mat_Save() {
  if (tableDate.name == "") {
    ...
    mat.matmajor = matToString(tableDate.matrix[0])
    mat.matminor = matToString(tableDate.basic_matrix[0])
    mat.matname = tableDate.name
    sqlApi.saveMat(mat).then(res => {
      var result = global.xmlToObject(res.data)
      if(result.retcode == 0)  window.alert("保存成功");
      else { window.alert("保存失败，错误原因："+result.result.message) }
    })
  }
...
function matToString(table) {  //将矩阵内容转换成字符串形式
  var result = table.row + "|" + table.col + "|";
  for (var i = 0; i < table.row; i++) {
    for (var j = 0; j < table.col; j++) {
      result += table.matrix[i][j] + "|";
    }
  }
  return result
}
...
function mat_Request() {
  ...
  eigen_temp = str;  //将str保存起来，用于获得长期行为中的特征值
  ...
  async function getData1(){
    try{
      ...
      await (async() => {
        var resultstr = global.xmlToValue(response.data);
        tableDate.return_m = [];
        //先按数组的列遍历，在案数组的行遍历，把每一列单独拿出来，做成每行只有一个元素的二维数组
        for (var col = 0; col < resultstr[1]; col++) {
          //这个地方输入了col和row
          tableDate.return_m.push({ row: resultstr[0], col: "1", matrix: [], eigen: null, constant: null, result: true });
          var matrix = []
  ...
  async function getData3(){
    ...
    for (var row = 0; row < tableDate.basic_matrix[0].row; row++) {
      ...
      await  (async() => {
        var resultstr = global.xmlToValue(response.data); //这里接口返回的是一个矩阵，因为这里接收的是常数，所以要跳过前两个值，他们为row和col
        //先按数组的列遍历，在案数组的行遍历，把每一列单独拿出来，做成每行只有一个元素的二维数组
